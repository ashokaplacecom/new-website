...
interface DashboardData {
  ...
  totalDegreeChanges: number;
  pendingVerifications: number;
  pendingDegreeChanges: number;
  approvedVerifications: number;
  approvedDegreeChanges: number;
  rejectedVerifications: number;
  rejectedDegreeChanges: number;
  totalPocs: number;
  topPocs: Array<{
    name: string;
    role: string;
    verificationCount: number;
    degreeChangeCount: number;
  }>;
}

export function DashboardClient({ data }: { data: DashboardData }) {
  const totalRequests = data.totalVerifications + data.totalDegreeChanges;
  const totalProcessed =
    data.approvedVerifications +
    data.rejectedVerifications +
    data.approvedDegreeChanges +
    data.rejectedDegreeChanges;
  const totalPending = data.pendingVerifications + data.pendingDegreeChanges;

  const pieData = [
    { name: "Verifications", value: data.totalVerifications, fill: "var(--color-chart-1)" },
    { name: "Degree Changes", value: data.totalDegreeChanges, fill: "var(--color-chart-3)" },
  ];
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Live statistics from the database.</p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Stat
          ...
          sub={`${data.totalVerifications} verifications · ${data.totalDegreeChanges} degree changes`}
          icon={FileCheck2}
        />
        <Stat
          label="Pending"
          value={totalPending.toString()}
          sub={`${data.pendingVerifications} verif. · ${data.pendingDegreeChanges} degree`}
          icon={Clock}
        />
        <Stat
          label="Processed"
          ...
          sub={totalRequests > 0
            ? `${Math.round((totalProcessed / totalRequests) * 100)}% completion rate`
            : "No requests yet"
          }
          icon={TrendingUp}
        />
      ...
      <div className="grid gap-4 lg:grid-cols-5">
        <GlassCard className="p-6 lg:col-span-2">
          ...
          {totalRequests > 0 ? (
            ...
            <div className="mt-4 flex items-center justify-center h-[260px] text-sm text-muted-foreground">
              No requests yet.
            </div>
          )}
        </GlassCard>
        ...
        <GlassCard className="p-6 lg:col-span-3">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">By total requests handled</p>
            </div>
          </div>
          {data.topPocs.length > 0 ? (
            <ol className="mt-4 divide-y divide-border/60">
              {data.topPocs.map((p, i) => {
                const total = p.verificationCount + p.degreeChangeCount;
                return (
            ...
            <div className="mt-4 flex items-center justify-center h-[200px] text-sm text-muted-foreground">
