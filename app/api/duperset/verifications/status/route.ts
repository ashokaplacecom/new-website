...
import { getLatestRequest } from '@/lib/supabase/db/requests'
...
export async function POST(req: NextRequest) {
    try {
        ...
        const request = await getLatestRequest(student.id)
        ...
        if (new Date(request.request_at) < oneWeekAgo) {
            return NextResponse.json({ success: true, data: null })
        }
        ...
        console.error('[POST /api/duperset/request/status]', err)
        return NextResponse.json(
...
export async function GET() {
    return NextResponse.json({ success: true, message: '/api/duperset/request/status endpoint running. Send POST with email.' })
}
