...
export function MarkdownRenderer({ content, className = "", variant = "default" }: MarkdownRendererProps) {
    const components: Components = {
        ...
        h1: ({ children, ...props }) => (
            <h1
                className={cn(
                    "font-serif font-bold tracking-tight text-foreground break-words",
                    variant === "landing"
                        ? "text-4xl md:text-6xl text-center mt-20 mb-8"
                        : "text-4xl md:text-5xl mt-12 mb-6 first:mt-0"
                )}
                {...props}
            >
                {children}
        ...
        h3: ({ children, ...props }) => (
            <h3
                className={cn(
                    "font-serif font-semibold text-foreground break-words",
                    variant === "landing"
                        ? "text-xl md:text-2xl text-center mt-8 mb-4"
